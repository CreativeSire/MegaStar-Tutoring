import Image from "next/image";
import Link from "next/link";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";

type Highlight = {
  title: string;
  description: string;
};

type RelatedRoute = {
  href: string;
  title: string;
  summary: string;
  image: string;
  accent?: string;
};

type MarketingRouteShellProps = {
  eyebrow: string;
  title: string;
  summary: string;
  image: string;
  accent: string;
  primaryCta: { label: string; href: string };
  secondaryCta: { label: string; href: string };
  highlights: ReadonlyArray<Highlight>;
  relatedRoutes: ReadonlyArray<RelatedRoute>;
  closingTitle: string;
  closingSummary: string;
};

export function MarketingRouteShell({
  eyebrow,
  title,
  summary,
  image,
  accent,
  primaryCta,
  secondaryCta,
  highlights,
  relatedRoutes,
  closingTitle,
  closingSummary,
}: MarketingRouteShellProps) {
  return (
    <div className="bg-white">
      <section className="bg-gradient-to-b from-navy-50 via-white to-coral-50/50 pt-32 pb-20">
        <div className="container-premium">
          <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="max-w-2xl">
              <span className="eyebrow mb-6">{eyebrow}</span>
              <h1 className="text-display-2 mb-6 text-navy">{title}</h1>
              <p className="text-body-large max-w-2xl text-navy-500">{summary}</p>

              <div className="mt-8 flex flex-wrap gap-3">
                <Button asChild size="lg" className="rounded-full">
                  <Link href={primaryCta.href}>{primaryCta.label}</Link>
                </Button>
                <Button
                  asChild
                  size="lg"
                  variant="outline"
                  className="rounded-full border-navy-200 bg-white text-navy hover:bg-navy-50"
                >
                  <Link href={secondaryCta.href}>{secondaryCta.label}</Link>
                </Button>
              </div>
            </div>

            <Card className="overflow-hidden border-navy-100 bg-white shadow-strong">
              <div className="relative h-[360px]">
                <Image src={image} alt={eyebrow} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" priority />
                <div className="absolute inset-0 bg-gradient-to-t from-navy/70 via-navy/25 to-transparent" />
                <div className="absolute left-6 top-6">
                  <Badge className="rounded-full bg-white/90 px-4 py-2 text-navy shadow-soft">
                    Dedicated page
                  </Badge>
                </div>
                <div className="absolute bottom-6 left-6 right-6 rounded-[24px] bg-white/92 p-5 shadow-soft backdrop-blur">
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-coral">What this page adds</p>
                  <p className="mt-2 text-lg font-semibold text-navy">{closingSummary}</p>
                </div>
              </div>
            </Card>
          </div>
        </div>
      </section>

      <section className="section-padding bg-white">
        <div className="container-premium">
          <div className="section-header mb-10">
            <span className="eyebrow">Key points</span>
            <h2>What this page helps you click into</h2>
            <p>Each card opens a more specific path or confirms why this route exists.</p>
          </div>

          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {highlights.map((item) => (
              <Card key={item.title} className="border-navy-100 shadow-soft">
                <CardContent className="space-y-3 p-6">
                  <p className="text-sm font-semibold uppercase tracking-[0.24em]" style={{ color: accent }}>
                    {item.title}
                  </p>
                  <p className="text-sm leading-6 text-navy-500">{item.description}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>

      <section className="section-padding bg-navy-50">
        <div className="container-premium">
          <div className="section-header mb-10">
            <span className="eyebrow">More routes</span>
            <h2>Keep moving through the product</h2>
            <p>The point of a multi-page site is that there is always a useful next page.</p>
          </div>

          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {relatedRoutes.map((route) => (
              <Card key={route.href} className="overflow-hidden border-navy-100 shadow-soft transition-transform hover:-translate-y-1">
                <div className="relative h-40">
                  <Image src={route.image} alt={route.title} fill sizes="(max-width: 1200px) 100vw, 25vw" className="object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-navy/70 via-navy/10 to-transparent" />
                </div>
                <CardContent className="space-y-4 p-6">
                  <p className="text-sm font-semibold uppercase tracking-[0.24em]" style={{ color: route.accent ?? accent }}>
                    Route
                  </p>
                  <div className="space-y-2">
                    <h3 className="text-xl font-bold text-navy">{route.title}</h3>
                    <p className="text-sm leading-6 text-navy-500">{route.summary}</p>
                  </div>
                  <Button asChild variant="outline" className="w-full rounded-full border-navy-200 bg-white text-navy hover:bg-navy-50">
                    <Link href={route.href}>Open page</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>

      <section className="bg-white pb-20">
        <div className="container-premium">
          <div className="rounded-[32px] border border-navy-100 bg-white p-8 shadow-soft">
            <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                <span className="eyebrow mb-4">Next action</span>
                <h2 className="text-heading-1 text-navy">{closingTitle}</h2>
                <p className="mt-4 max-w-2xl text-navy-500 leading-7">{closingSummary}</p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                <Button asChild size="lg" className="rounded-full">
                  <Link href={primaryCta.href}>{primaryCta.label}</Link>
                </Button>
                <Button asChild size="lg" variant="outline" className="rounded-full border-navy-200 bg-white text-navy hover:bg-navy-50">
                  <Link href="/pricing">View pricing</Link>
                </Button>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
